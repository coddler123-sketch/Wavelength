...
| `src/stations.js` | Firebase station loading + in-memory cache |
...
| `src/firebase-config.js` | Firebase Realtime DB connection (requires environment config) |
...
1. Firebase config in `src/firebase-config.js` (requires valid project credentials)
2. `src/stations.js` loads stations on app start
3. Renderer receives list via IPC 'stations-loaded'
4. Click station → emits IPC 'select-station' with station object
