...
function genreTagPriority(tag) {
  ...
  if (!t || ['radio', 'music', 'ard', 'live', 'non-stop', 'berlin', 'bayerischer rundfunk'].includes(t))
    return 0;
  if (
    t.includes('culture') ||
    t.includes('kultur') ||
    ...
    return 100;
  if (t.includes('news') || t.includes('information') || t.includes('talk') || t.includes('public radio'))
    return 100;
  if (
    t.includes('hiphop') ||
    t.includes('hip hop') ||
  ...
  if (
    ...
    t.includes('world') ||
    t.includes('global') ||
    t.includes('multicultural')
  )
    return 80;
  if (t.includes('jazz') || t.includes('classic') || t.includes('klassik') || t === 'acid') return 75;
  if (
    /\b(50s|60s|70s|80s|90s|00s|10s|1950s|1960s|1970s|1980s|1990s|2000s|2010s|50er|60er|70er|80er|90er|2000er|2010er)\b/.test(
      t
    ) ||
    t.includes('oldies') ||
    t.includes('retro')
    ...
    return 70;
  if (t.includes('rock') || t.includes('metal') || t.includes('alternative') || t.includes('indie'))
    return 78;
  if (
    t.includes('pop') ||
    t.includes('charts') ||
...
function localizeLanguageLabel(language, country = '', name = '', website = '', streamUrl = '') {
  ...
  if (
    countryCode === 'DE' &&
    (values.length === 0 || values.includes('german') || values.includes('deutsch') || values.includes('de'))
  ) {
    return 'Deutsch';
  }
...
function normalizeStationLanguage(station) {
  return {
    ...station,
    language: localizeLanguageLabel(
      station.language,
      station.country,
      station.name,
      station.website,
...
function normalizeStationGenre(station) {
  ...
  return {
    ...station,
    genre:
      inferredGenre && weakGenres.has(normalizedGenre)
        ? inferredGenre
...
const HIGH_RES_ICONS = {
  'deutschlandfunk kultur':
    'https://www.deutschlandfunkkultur.de/static/img/deutschlandfunk_kultur/icons/apple-touch-icon-128x128.png',
  ...
  'ndr info':
    'https://images.ndr.de/image/e03539c6-eebc-482a-911f-4b4d115c1c7d/AAABnoyc4w4/AAABnSSvrFg/16x9-big/logoinfotv100.webp?width=1920',
